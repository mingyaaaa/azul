...
use azul_css::{
    ...
    LayoutAxis, LayoutPosition, RectLayout, LayoutPoint,
};
...
macro_rules! determine_preferred {
    ...
    fn $fn_name(layout: &RectLayout, preferred_inner_width: Option<f32>) -> WhConstraint {
        ...
        let width = layout.$width.map(|w| w.inner.to_pixels());
        let min_width = layout.$min_width.map(|w| w.inner.to_pixels());
        let max_width = layout.$max_width.map(|w| w.inner.to_pixels());
...
impl WidthCalculatedRect {
    ...
    fn get_flex_basis_horizontal(&self) -> f32 {
    ...
    fn solved_result(&self) -> WidthSolvedResult {
        WidthSolvedResult {
...
struct HeightCalculatedRect {
    preferred_height: WhConstraint,
    margin: LayoutMargin,
    padding: LayoutPadding,
    flex_grow_px: f32,
    min_inner_size_px: f32,
}

impl HeightCalculatedRect {
    /// Get the flex basis in the horizontal direction - vertical axis has to be calculated differently
    fn get_flex_basis_vertical(&self) -> f32 {
        ...
        loop {
            ...
            let children_combined_flex_grow: f32 = variable_width_childs
                ...
                .map(|child_id|
                        ...
                        arena_data[*child_id].flex_grow
                            .map(|grow| grow.inner.get().max(1.0))
            ...
            for variable_child_id in &variable_width_childs {
                ...
                let flex_grow = arena_data[*variable_child_id].flex_grow
                    .and_then(|grow| Some(grow.inner.get()))
                    .unwrap_or(DEFAULT_FLEX_GROW_FACTOR);
...
#[derive(Debug, Clone)]
struct SolvedWidthLayout {
    solved_widths: NodeDataContainer<WidthSolvedResult>,
    layout_only_arena: NodeDataContainer<RectLayout>,
    non_leaf_nodes_sorted_by_depth: Vec<(usize, NodeId)>,
}

#[derive(Debug, Clone)]
struct SolvedHeightLayout {
    solved_heights: NodeDataContainer<HeightSolvedResult>,
}

/// Returns the solved widths of the items in a BTree form
fn solve_flex_layout_width(
    node_hierarchy: &NodeHierarchy,
...
struct HorizontalSolvedPosition(pub f32);

#[derive(Debug, Copy, Clone, PartialEq, PartialOrd)]
struct VerticalSolvedPosition(pub f32);
...
-> NodeDataContainer<$height_solved_position>
{
    fn determine_child_x_absolute(
        ...
        let child_left = &arena_data[child_id].$left.map(|s| s.inner.to_pixels());
        let child_right = &arena_data[child_id].$right.map(|s| s.inner.to_pixels());
